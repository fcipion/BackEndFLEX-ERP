        ...
        if(req.user){
            ...
            if(accesos != null)
            {
                if(accesos.acceso[0].read == true){
                    ...
                    let reg = await Producto.find({ compania: req.user.compania, descripcion: new RegExp(filtro, 'i') }).populate('sucursal').populate('almacen').populate('tipo_producto').populate('clase_producto').populate('itbis').populate('unidad_medida_compra').populate('unidad_medida_venta').populate('unidad_medida_inventario').populate('lista_precio_venta').populate('lista_precio_compra');
        ...
        if(req.user){
            ...
            if(accesos != null)
            {
                if(accesos.acceso[0].read == true){
                    var id = req.params['id'];
                    try {
                        var reg = await Producto.findById({_id:id}).populate('sucursal').populate('almacen').populate('tipo_producto').populate('clase_producto').populate('itbis').populate('unidad_medida_compra').populate('unidad_medida_venta').populate('unidad_medida_inventario').populate('lista_precio_venta').populate('lista_precio_compra');
        ...
        if(req.user){
            ...
            if(accesos != null)
            {
                if(accesos.acceso[0].read == true){
                    ...
                    let reg = await Determinacion_precio.find({ compania: req.user.compania, descripcion: new RegExp(filtro, 'i') }).populate({ path: 'detalles', populate: { path: 'unidad_medida' } }).populate({ path: 'detalles', populate: { path: 'producto' }}).populate({path: 'detalles', populate: { path: 'moneda' }}).populate('sucursal').populate('lista_precio');
        ...
        if(req.user){
            ...
            if(accesos != null)
            {
                if(accesos.acceso[0].read == true){
                    var id = req.params['id'];
                    try {
                        var reg = await Determinacion_precio.findById({_id:id}).populate({ path: 'detalles', populate: { path: 'unidad_medida' } }).populate({ path: 'detalles', populate: { path: 'producto' }}).populate({path: 'detalles', populate: { path: 'moneda' }}).populate('sucursal').populate('lista_precio');
